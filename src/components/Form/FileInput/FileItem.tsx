...
const fileItem = tv({
  slots: {
    ...
    icon: 'bordewr-4 rounded-full border-x-violet-100 bg-violet-200 p-2 text-violet-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500',
...
export function FileItem({ name, size, state }: FileItemProps) {
  const { deleteButton, icon, container } = fileItem({ state })
